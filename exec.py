...
class ExecCommand(sublime_plugin.WindowCommand, ProcessListener):
    ...
    def run(
        ...
        if query_input_args:
            ...
            self.fun(cmd, shell_cmd, merged_env)

    def fun(self, cmd, shell_cmd, merged_env, input_args=None):

        if input_args is not None:
            history.insert(input_args)
            if shell_cmd:
                shell_cmd += " " + str(input_args)
            else:
                cmd.append(str(input_args))
        ...
        try:
            ...
            self.proc = AsyncProcess(cmd, shell_cmd, merged_env, self, self.kwargs)
